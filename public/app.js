...
function updateDynamicScriptCommands(workerName) {
  ...
  // Atualiza retângulo adaptável do Colab Multi-Instância
  const colabFullEl = document.getElementById('colabFullCodeBox');
  if (colabFullEl) colabFullEl.value = getColabFullScriptText(name);
...
function getColabFullScriptText(operatorName) {
  const host = window.location.host || 'puzzleradar-production.up.railway.app';
  const protocol = window.location.protocol === 'https:' ? 'https' : 'http';
  const baseUrl = `${protocol}://${host}`;
...
function copyColabFullScript(btnElement) {
  const colabFullEl = document.getElementById('colabFullCodeBox');
  const code = colabFullEl ? colabFullEl.value : getColabFullScriptText(getActiveWorkerNickname());
  copyToClipboard(code, btnElement);
  showToast('📋 Código Python Multi-Instância para Google Colab copiado com sucesso!');
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Inicializa todos os comandos na tela com valores padrão
  updateDynamicScriptCommands();
});
