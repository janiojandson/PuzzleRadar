...
async function checkMutations() {
  ...
  const data = await callRailway(schemaQuery);
  if (data.data && data.data.__schema) {
    ...
    const relevant = fields.filter(f => 
      ...
      f.name.toLowerCase().includes('env') ||
      f.name.toLowerCase().includes('service')
    );

    console.log('Mutations disponíveis relevantes:');
    for (const f of relevant) {
      console.log(`- ${f.name}(${f.args.map(a => a.name).join(', ')})`);
    }
  } else {
    console.log('Erro ao consultar schema:', data);
  }
}
